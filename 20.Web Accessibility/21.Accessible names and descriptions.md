...
In addition to an accessible name that identifies a component, some user interface components may benefit from a ‘description’ or an ‘accessible description’.

An accessible description (accDesc) is similar to an accessible name in that:

- it also helps the user understand how to use a component, and
- it is exposed by the browser to assistive technologies in the accessibility tree when it is programmatically associated with an element.

### The difference between a description and a name

An accessible name is usually short and succinct. Whereas an accessible description is often longer, and provides additional cues that may describe how to use the component. For example, form field requirements and error messages provide additional information to the user about the type of input that the input field expects. These descriptions can be programmatically associated with the input field using the `aria-describedby` attribute:
...
The accName of the password field in this example is “Password”. And the password hint provides an additional accessible description that helps the user fill a password in the field.

The accDesc of an element is appended to the name and role of the element and is presented to the user as a flat string of text. If the description contains structured content such as lists or links, the structure and semantics are lost when the content is announced to the user. We will elaborate more on this in the next chapter.

Accessible descriptions also differ from accessible names in how they are announced by screen readers. Accessible names are always announced by screen readers. But descriptive text [may or may not be announced.](https://adrianroselli.com/2022/04/accessible-description-exposure.html) We will also elaborate more on this in the next chapter.

Moreover, _how_ a description is announced depends on the browser and screen reader combination used, as well as the method you use to associate the description with its element. _When_ a description is announced is also a function of the user’s interaction type. In most cases, if the descriptive text is announced, it is appended and announced _after_ the accessible name and role of an element.

## Outro

In the next chapter, we’re going to learn about different techniques that you can use to provide accessible names to elements, and the different ways you can use to associate descriptive text with it.
