...
export function FoodItemsWithController() {
...
export function OrderItemRow({
    ...
    function onChangeFood(evt: TSelectEvt, rowIndex: number) {
        const foodId = parseInt(evt.target.value?.toString());
        let price: number = 0;
        if (foodId > 0) {
    ...
    function onChangeQuantity(rowIndex: number, evt?: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) {
            ...
            <td colSpan={2}>
                <Controller
                    ...
                    render={({ field, fieldState }) => <>
                        {foodOptions && <FormControl
                            disabled={isSubmitting}
                            fullWidth
                            error={!!fieldState.error}>
                            <InputLabel id={`foodId${index}`}> food #{index}</InputLabel>
                            <Select fullWidth label={`food #${index}`} labelId={`foodId${index}`} {...field} onChange={(evt) => {
                                field.onChange(evt);
